...
class UnetHook(nn.Module):
    ...
    def hook(self, model, sd_ldm, control_params, process):
        ...
        model_is_sxdxl = getattr(self.sd_ldm, 'is_sxdxl', False)
        ...
        def forward(self, x, timesteps=None, context=None, y=None, **kwargs):
            is_sxdxl = y is not None and model_is_sxdxl
            ...
            for param in outer.control_params:
                ...
                if param.control_model is not None:
                    if outer.lowvram and is_sxdxl and hasattr(param.control_model, 'aggressive_lowvram'):
            ...
            for param in outer.control_params:
                ...
                if is_sxdxl and param.control_model_type == ControlModelType.ControlNet:
            ...
            if shared.cmd_opts.medvram or (getattr(shared.cmd_opts, 'medvram_sxdxl', False) and is_sxdxl):
                try:
            ...
            for param in outer.control_params:
                ...
                if is_sxdxl:
                    # sxdxl's attention hacking is highly unstable.
            ...
            with th.no_grad():
                ...
                for i, module in enumerate(self.input_blocks):
                    ...
                    t2i_injection = [3, 5, 8] if is_sxdxl else [2, 5, 8, 11]
            ...
            if len(total_t2i_adapter_embedding) > 0 and is_sxdxl:
                h = aligned_adding(h, total_t2i_adapter_embedding.pop(0), require_inpaint_hijack)
        ...
        def forward_startfk(*args, **kwargs):
            # startfk will handle other compoments 
            try:
        ...
        model.forward = forward_startfk.__get__(model, UNetModel)
