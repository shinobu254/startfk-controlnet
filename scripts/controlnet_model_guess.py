...
controlnet_sxdxl_config = {'num_classes': 'sequential',
...
controlnet_sxdxl_mid_config = {'num_classes': 'sequential',
                              'adm_in_channels': 2816,
                              'in_channels': 4,
                              'model_channels': 320,
                              'num_res_blocks': 2,
...
controlnet_sxdxl_small_config = {'num_classes': 'sequential',
                                'adm_in_channels': 2816,
                                'in_channels': 4,
                                'model_channels': 320,
                                'num_res_blocks': 2,
...
def build_model_by_guess(state_dict, unet, model_path):
    if "lora_controlnet" in state_dict:
        ...
        config = copy.deepcopy(controlnet_sxdxl_config)
        logger.info('controlnet_sxdxl_config (using lora)')
    ...
    if 'input_hint_block.0.weight' in state_dict:
        if 'label_emb.0.0.bias' not in state_dict:
            ...
            if has_full_layers:
                config = copy.deepcopy(controlnet_sxdxl_config)
                logger.info('controlnet_sxdxl_config')
            elif has_mid_layers:
                config = copy.deepcopy(controlnet_sxdxl_mid_config)
                logger.info('controlnet_sxdxl_mid_config')
            else:
                config = copy.deepcopy(controlnet_sxdxl_small_config)
                logger.info('controlnet_sxdxl_small_config')
            config['global_average_pooling'] = False
            config['hint_channels'] = int(state_dict['input_hint_block.0.weight'].shape[1])
